...
document.getElementById('contact-page-submit').addEventListener('click', () => {
	const formInputs = document.querySelectorAll('.contact-container input, .contact-container textarea');
	...
	const templateParams = {
		...
		message_html: document.getElementById('yourstory-input').value,
		client_phone: document.getElementById('phone-input').value = "" ? document.getElementById('phone-input').value : "Not Provided",
	};

	document.getElementById('contact-page-submit').innerHTML = 'Sending';

			emailjs.send('gmail', 'contact_page_message', templateParams, 'user_Cyzt5zhpOEbvOcpYYLBlc')
			.then((response) => {
				console.log('SUCCESS!', response.status, response.text);
				document.getElementById('contact-page-submit').innerHTML = 'Email Sent';
			}, (err) => {
				console.log('FAILED...', err);
				document.getElementById('contact-page-submit').innerHTML = 'Please Try Again';
			});
		}
});
